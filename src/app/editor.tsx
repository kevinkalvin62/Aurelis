...
export default function EditorScreen() {
  ...
  const { control, handleSubmit, setValue, formState: { errors, isDirty } } = useForm<FormValues>({
    defaultValues: {
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
      ...
      <Field control={control} name="title" placeholder="Título de la canción" large error={Boolean(errors.title)} />
      <Field control={control} name="artist" placeholder="Artista o autor" error={Boolean(errors.artist)} />

      <Text style={styles.label}>TIPO DE CONTENIDO</Text>
      <View style={styles.typeGrid}>{contentTypes.map((option) => <Pressable key={option.value} onPress={() => chooseContentType(option.value)} style={[styles.typeCard, contentType === option.value && styles.typeCardActive]}><Text style={[styles.typeMark, contentType === option.value && styles.typeTextActive]}>{option.mark}</Text><Text style={[styles.typeLabel, contentType === option.value && styles.typeTextActive]}>{option.label}</Text></Pressable>)}</View>
...
function Field({ control, name, placeholder, large, centered, error }: { control: ReturnType<typeof useForm<FormValues>>['control']; name: 'title' | 'artist' | 'key'; placeholder: string; large?: boolean; centered?: boolean; error?: boolean }) {
  return <Controller control={control} name={name} render={({ field: { value, onChange, onBlur } }) => <TextInput value={value} onChangeText={onChange} onBlur={onBlur} placeholder={placeholder} placeholderTextColor="#6E6864" style={[styles.input, large && styles.inputLarge, centered && { textAlign: 'center' }, error && styles.error]} />} />;
}

const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: colors.background }, nav: { height: 64, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border }, cancel: { color: colors.textSecondary, fontSize: 13 }, navTitle: { color: colors.text, fontSize: 14, fontWeight: '700' }, scroll: { padding: spacing.lg, paddingBottom: 80, width: '100%', maxWidth: 760, alignSelf: 'center' }, eyebrow: { color: colors.accent, fontSize: 9, fontWeight: '900', letterSpacing: 1.6, marginBottom: 14 },
