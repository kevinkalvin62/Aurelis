...
function transposeNote(note: string, semitones: number, preference: AccidentalPreference): string {
  ...
  const notes = preference === 'flat' || (preference === 'auto' && note.includes('b')) ? FLAT_NOTES : SHARP_NOTES;
...
export function transposeChord(
  ...
  const effectivePreference = preference === 'auto' && !root.includes('b') && semitones < 0 ? 'flat' : preference;
...
export function transposeSong(content: string, semitones: number, preference: AccidentalPreference = 'auto'): string {
  if (semitones === 0) return content;
  return content.replace(/\[([^\]\r\n]+)]/g, (_, chord: string) => `[${transposeChord(chord, semitones, preference)}]`);
}

export interface SongToken { type: 'chord' | 'lyric'; value: string }

export function tokenizeLine(line: string): SongToken[] {
  const tokens: SongToken[] = [];
  let cursor = 0;
  for (const match of line.matchAll(/\[([^\]]+)]/g)) {
    if (match.index > cursor) tokens.push({ type: 'lyric', value: line.slice(cursor, match.index) });
...
export function transposeKey(key: string, semitones: number, preference: AccidentalPreference = 'auto'): string {
