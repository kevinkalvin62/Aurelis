  ...
  return (
    <Screen eyebrow="TU REPERTORIO" title="Biblioteca" subtitle={`${songs.length} canciones · ${accessMode === 'authenticated' ? 'sincronización activa' : 'guardadas localmente'}`} right={<Pressable onPress={() => router.push('/editor')} style={styles.add}><Text style={styles.addText}>＋</Text></Pressable>}>
      ...
      <View style={styles.list}>
        {visibleSongs.map((song, index) => <SongRow key={song.id} song={song} index={index + 1} />)}
        {visibleSongs.length === 0 ? <Text style={styles.empty}>No encontramos canciones con ese criterio.</Text> : null}
      </View>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  add: { width: 42, height: 42, borderRadius: 21, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' },
  addText: { color: colors.text, fontSize: 23, fontWeight: '300' },
  search: { height: 50, backgroundColor: colors.surface, borderRadius: radii.md, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, gap: 10 },
  ...
  filter: { paddingHorizontal: 13, paddingVertical: 8, borderRadius: radii.pill, borderWidth: 1, borderColor: colors.border },
  ...
  heading: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, marginBottom: 4 },
  count: { color: colors.textSecondary, fontSize: 9, fontWeight: '800', letterSpacing: 1.4 },
  sort: { color: colors.textSecondary, fontSize: 10 },
  list: { backgroundColor: colors.surface, borderRadius: radii.md, paddingHorizontal: spacing.md, borderWidth: 1, borderColor: colors.border },
