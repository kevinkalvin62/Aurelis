interface SetlistSourcePayload {
  version: 1;
  sourceText?: string;
  notes?: string;
}
...
export function encodeSetlistSource(sourceText?: string, notes?: string): string | null {
  ...
  const payload: SetlistSourcePayload = {
    version: 1,
...
export function decodeSetlistSource(value: unknown): { sourceText?: string; notes?: string } {
  ...
  try {
    ...
    if (parsed.version === 1)
      return {
        ...(typeof parsed.sourceText === "string" ? { sourceText: parsed.sourceText } : {}),
        ...(typeof parsed.notes === "string" ? { notes: parsed.notes } : {}),
      };
  } catch {
    /* Legacy source_text remains plain text. */
  }
