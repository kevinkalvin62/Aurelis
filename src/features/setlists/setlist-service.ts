...
export async function createRemoteSetlist(input: { organizationId: string; title: string; serviceDate?: string; notes?: string; sourceText?: string; items: SetlistDraftItem[] }): Promise<{ id?: string; error?: string }> {
  if (input.serviceDate && !isISODate(input.serviceDate)) return { error: 'La fecha no es válida.' };
  ...
  if (input.items.length) {
    const rows = input.items.map((item, position) => ({ setlist_id: id, song_id: item.songId || null, source_title: item.titleSnapshot, title_snapshot: item.titleSnapshot, position, selected_key: item.selectedKey || null, notes: item.notes || '' }));
