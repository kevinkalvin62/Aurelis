...
export default function SongScreen() {
  ...
  const content = useMemo(() => transposeContent(song.content, semitones, song.contentType, song.notation), [semitones, song]);
  const displayLines = useMemo(() => getDisplayLines(content, song.contentType, song.notation), [content, song.contentType, song.notation]);
  const key = transposeNotationChord(song.key, semitones, song.notation);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={[styles.scroll, presentationMode && styles.presentation]} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.note}>NOTA DEL DIRECTOR</Text><View style={styles.directorNote}><View style={styles.noteRule} /><Text style={styles.noteCopy}>Entrar suave en el verso. Subir dinámica después del segundo coro y cerrar sólo con piano.</Text></View>
      </ScrollView>
