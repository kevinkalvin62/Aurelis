...
const song = (id: string, title: string): Song => ({ id, title, artist: '', key: 'C', bpm: 80, visibility: 'private', updatedAt: '', content: '', contentType: 'lyrics_chords', notation: 'american' });
...
describe('parsePastedSetlist', () => {
  ...
  it('extracts a title and exact normalized matches', () => {
    const parsed = parsePastedSetlist('Domingo AM\n- Digno y Santo\n2. Gracia Sublime', library);
  ...
  it('handles accents and leaves unknown songs unconfirmed', () => {
    const parsed = parsePastedSetlist('Ensayo\nGrácia Sublime\nCanción inexistente', library);
