...
interface SetlistState {
  ...
  createSetlist: (input: { title: string; serviceDate?: string; notes?: string; sourceText?: string; items: SetlistDraftItem[] }) => Setlist;
...
export const useSetlistStore = create<SetlistState>()(persist(
  (set) => ({
    ...
    createFromMessage: (lines, songIds) => {
      const created = buildSetlist({ title: lines[0] || 'Nuevo programa', sourceText: lines.join('\n'), items: lines.slice(1).map((titleSnapshot, index) => ({ titleSnapshot, ...(songIds[index] ? { songId: songIds[index] } : {}) })) });
    ...
    linkItem: (setlistId, itemId, songId) => set((state) => ({ setlists: state.setlists.map((setlist) => {
      if (setlist.id !== setlistId) return setlist;
      if (!setlist.items) return setlist;
      const items = setlist.items.map((item) => item.id === itemId ? { ...item, songId } : item);
      return { ...setlist, items, songIds: items.flatMap((item) => item.songId ? [item.songId] : []) };
    }) })),
