...
interface AuthForm { email: string; password: string }
...
function friendlyAuthError(message: string): string {
  ...
  if (normalized.includes('user already registered')) return 'Este correo ya tiene una cuenta. Inicia sesión.';
...
export default function AuthScreen() {
  ...
  const { control, handleSubmit, getValues, formState: { errors } } = useForm<AuthForm>({ defaultValues: { email: '', password: '' } });

  const enterAsGuest = () => { continueAsGuest(); router.replace('/'); };
  const submit = handleSubmit(async ({ email, password }) => {
    if (!isSupabaseConfigured) return setMessage('La conexión con Supabase no está configurada. Puedes continuar como invitado.');
    setLoading(true); setMessage(''); setSuccess(false);
    const result = mode === 'login'
      ? await supabase.auth.signInWithPassword({ email: email.trim(), password })
      : await supabase.auth.signUp({ email: email.trim(), password, options: { data: { display_name: 'Kevin' } } });
    setLoading(false);
    if (result.error) return setMessage(friendlyAuthError(result.error.message));
    if (result.data.session?.user) {
      const signedUser = result.data.session.user;
      setAuthenticated({ id: signedUser.id, email: signedUser.email ?? email, name: 'Kevin' });
      router.replace('/');
      return;
    }
    setSuccess(true); setMessage('Cuenta creada. Revisa tu correo para confirmarla y después inicia sesión.');
  }, () => setMessage('Escribe un correo válido y una contraseña de al menos 8 caracteres.'));

  const resetPassword = async () => {
    const email = getValues('email').trim();
    if (!email.includes('@')) return setMessage('Escribe primero tu correo electrónico.');
    setLoading(true); const { error } = await supabase.auth.resetPasswordForEmail(email); setLoading(false);
    setSuccess(!error); setMessage(error ? friendlyAuthError(error.message) : 'Te enviamos un enlace para recuperar tu contraseña.');
  };

  const signOut = async () => {
    setLoading(true); await supabase.auth.signOut(); clearAccess(); setLoading(false); setMessage('Sesión cerrada. Tus datos locales siguen en este dispositivo.');
  };

  if (accessMode === 'authenticated' && user) return <SafeAreaView style={styles.safe}><View style={styles.container}>
    <Pressable onPress={() => router.back()} style={styles.back}><Text style={styles.backText}>‹</Text></Pressable>
    <View style={styles.brand}><View style={styles.mark}><Text style={styles.markText}>A</Text></View><Text style={styles.wordmark}>Aurelis</Text></View>
    <View style={styles.accountCard}><Text style={styles.accountEyebrow}>CUENTA CONECTADA</Text><Text style={styles.accountName}>{user.name}</Text><Text style={styles.copy}>{user.email}</Text><Text style={styles.syncCopy}>Tus canciones locales se sincronizan con Supabase cuando hay conexión.</Text>{loading ? <ActivityIndicator color={colors.accent} /> : <Button label="Cerrar sesión" variant="secondary" onPress={signOut} />}</View>
  </View></SafeAreaView>;
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.container}>
    ...
    <View style={styles.brand}><View style={styles.mark}><Text style={styles.markText}>A</Text></View><Text style={styles.wordmark}>Aurelis</Text><Text style={styles.tagline}>Tu repertorio. Una sola verdad.</Text></View>
