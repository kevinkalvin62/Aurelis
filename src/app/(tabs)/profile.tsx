...
export default function ProfileScreen() {
  ...
  const createChurch = () => { if (accessMode !== 'authenticated') { toast.warning('Necesitas iniciar sesión para usar esta función.'); router.push('/auth'); return; } router.push('/organization/create'); };
  return (
    <Screen eyebrow="CUENTA Y EQUIPO" title="Perfil">
      ...
      {accessMode !== 'authenticated' ? <Pressable onPress={createChurch} style={styles.org}><View style={styles.orgMark}><Text style={styles.orgLetter}>A</Text></View><View style={{ flex: 1 }}><Text style={styles.rowTitle}>Función disponible con cuenta</Text><Text style={styles.rowCopy}>Inicia sesión para crear o unirte a una iglesia</Text></View><Text style={styles.chevron}>›</Text></Pressable> : <View style={styles.group}>
        ...
        <Pressable onPress={createChurch} style={styles.row}><Text style={styles.addMark}>＋</Text><Text style={[styles.rowTitle, { flex: 1 }]}>Crear iglesia</Text><Text style={styles.chevron}>›</Text></Pressable>
      </View>}
