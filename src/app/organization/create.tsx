...
export default function CreateOrganizationScreen() {
  ...
  const changeName = (value: string) => {
    setName(value);
    if (!editedSlug) setSlug(slugifyOrganization(value));
  };
  const submit = async () => {
    ...
    if (name.trim().length < 2 || slug.length < 2) {
      toast.error("Escribe un nombre válido para la organización.");
      return;
    }
    setSaving(true);
    ...
    setSaving(false);
    if (!result.data) {
      toast.error(result.error ?? "No fue posible crear la organización.");
      return;
    }
      ...
      <View style={styles.content}>
        ...
        <View style={styles.typeOptions}>
          {organizationTypes.map((option) => (
            ...
            >
              <Text style={[styles.typeText, type === option.value && styles.typeTextActive]}>
                {option.label}
              </Text>
            </Pressable>
          ))}
        ...
        <View style={styles.slugRow}>
          ...
          <TextInput
            ...
            onChangeText={(value) => {
              setEditedSlug(true);
              setSlug(slugifyOrganization(value));
            }}
        ...
        <Text style={styles.hint}>
          Se usa para identificar tu organización. Puedes cambiar el nombre visible después.
        </Text>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  at: { color: colors.accent, fontSize: 16, fontWeight: "800" },
  slugInput: { flex: 1, color: colors.text, paddingHorizontal: 8, fontSize: 14 },
  hint: { color: "#77706C", fontSize: 10, lineHeight: 15, marginTop: 8 },
});
