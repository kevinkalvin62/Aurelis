    ...
    (set, get) => ({
      ...
      saveSong: (draft, existingId) => {
        const previous = existingId
          ? get().songs.find((song) => song.id === existingId)
          : undefined;
        const saved: Song = {
          ...draft,
      ...
      markSyncPending: (id) =>
        set((state) => ({
          songs: state.songs.map((song) =>
            song.id === id ? { ...song, syncStatus: "pending" } : song,
          ),
        })),
      markSynced: (id, remoteId) =>
        set((state) => ({
          songs: state.songs.map((song) =>
            song.id === id ? { ...song, remoteId, syncStatus: "synced" } : song,
          ),
        })),
      mergeRemoteSongs: (remoteSongs) =>
        set((state) => {
      ...
      restoreSong: (song) =>
        set((state) => ({
          songs: state.songs.map((item) => (item.id === song.id ? song : item)),
        })),
    }),
    {
      name: "aurelis:inactive:songs",
