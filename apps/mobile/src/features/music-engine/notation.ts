...
const LATIN_CHORD =
  /^(DO|RE|MI|FA|SOL|LA|SI)([#b♯♭]?)([^/]*?)(?:\/(DO|RE|MI|FA|SOL|LA|SI)([#b♯♭]?))?$/i;
const BAR_TOKEN = /^(?:\|{1,2}|\/{1,2}|:|x\d+)$/i;

function modulo(value: number): number {
  return ((value % 12) + 12) % 12;
}
function normalizeAccidental(value = ""): string {
...
export function transposeNotationChord(
  ...
  const wantsFlats =
    preference === "flat" || (preference === "auto" && (root.includes("b") || semitones < 0));
  const notes =
    notation === "latin"
      ? wantsFlats
        ? FLAT_LATIN
  ...
  const bass = bassName
    ? `${bassName.toUpperCase()}${normalizeAccidental(bassAccidental)}`
    : undefined;
...
export function transposeMelody(
  ...
  if (notation === "latin") {
    return content.replace(/SOL[#b]?|DO[#b]?|RE[#b]?|MI[#b]?|FA[#b]?|LA[#b]?|SI[#b]?/gi, (note) =>
      transposeNotationChord(note, semitones, "latin"),
    );
  }
  return content.replace(/\S+/g, (segment) => {
    if (/[a-z]/.test(segment) || !/^[A-G#b/.\-]+$/.test(segment)) return segment;
    return segment.replace(/[A-G](?:#|b)?/g, (note) =>
      transposeNotationChord(note, semitones, "american"),
    );
  });
}
...
export function transposeContent(
  ...
  if (content.includes("[")) {
    return content.replace(
      /\[([^\]\r\n]+)]/g,
      (_, chord: string) => `[${transposeNotationChord(chord, semitones, notation)}]`,
    );
  }
  return content
    .split("\n")
...
export function inlineToAlignedLines(line: string): [string, string] {
  ...
  for (let index = 0; index < line.length;) {
    if (line[index] === "[") {
      ...
      if (close >= 0) {
        pendingChord = line.slice(index + 1, close);
        index = close + 1;
        continue;
      }
    }
...
export function getDisplayLines(
  ...
  if (contentType === "lyrics_chords" && content.includes("[")) {
    return content.split("\n").flatMap((line): DisplayLine[] => {
      ...
      return [
        { kind: "chord", value: chord },
        { kind: "lyric", value: lyric },
      ];
    });
  }
  return content.split("\n").map((value) => ({
    kind: !value
      ? "space"
      : contentType === "chords_only" || isChordLine(value, notation)
        ? "chord"
        : "lyric",
    value,
  }));
