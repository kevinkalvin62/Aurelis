...
export interface ParsedSetlistLine {
  line: string;
  song?: Song;
}
...
export function parsePastedSetlist(
  source: string,
  songs: Song[],
): { title: string; matches: ParsedSetlistLine[] } {
  const lines = source
    .split(/\r?\n/)
  ...
  const first = lines[0] ?? "";
  const hasHeader =
    /:$/.test(first) || /^(lista|programa|setlist|ensayo|domingo|servicio|evento)\b/i.test(first);
